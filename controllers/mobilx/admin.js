...
exports.read = async function (ctx) {
    let admin = await models.mobilx.admin.getByUid(ctx, Number(ctx.admin.uid))
...
exports.changePassword = async function (ctx) {
    let admin = await models.mobilx.admin.getById(ctx.admin.id, models.mobilx)
    let _ = ctx.request.body
...
exports.login = async function (ctx) {
    let _ = ctx.request.body
    let admin = await models.mobilx.admin.getById(_.id, models.mobilx)
    if (!admin) {
        ctx.throw({
            code: 400,
    ...
    const accessToken = jwt.sign(
        {
            ...
            name: admin.name,
            authType: "admin",
            companyUid: admin.companyUid,
            companyName: admin.company ? admin.company.name : null
        },
        secret
    )
    response.send(ctx, {
